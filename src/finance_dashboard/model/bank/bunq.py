...
class Bunq:
    """Bunq banking API client for retrieving account information."""
    ...
    def __init__(self, api_key: str, configuration_file: str):
        ...
        try:
            ...
            self.logger.exception(f"Failed to initialize Bunq API context: {error_details}")
            raise

    def retrieve_accounts(self):
        """Retrieve all active bank accounts from Bunq API."""
        try:
            ...
            with warnings.catch_warnings():
                ...
                try:
                    ...
                    if "float() argument must be a string or a real number, not 'NoneType'" in str(e):
                        self.logger.warning(
                            "Bunq API returned malformed data with null values. Trying with smaller batch size..."
                        )
                        # Try with smaller batch size to see if it helps
                        monetary_accounts = MonetaryAccountApiObject.list(params={"count": "50"})
                        self.logger.debug(
                            f"Retrieved {len(monetary_accounts.value)} monetary accounts with smaller batch"
                        )
                    else:
                        raise
                ...
                for i, monetary_account in enumerate(monetary_accounts.value):
                    try:
                        ...
                        if account is not None and account.status == "ACTIVE":
                            ...
                            if (
                                account.balance
                                and hasattr(account.balance, "value")
                                and account.balance.value is not None
                            ):
                                try:
                                    balance_value = float(account.balance.value)
                                except (ValueError, TypeError):
                                    self.logger.warning(
                                        f"Invalid balance value for account {account.description}: {account.balance.value}. Using 0.0"
                                    )
                                    balance_value = 0.0

                            # Improved currency detection - get from balance object or account directly
                            currency = "EUR"  # Default fallback
                            if account.balance and hasattr(account.balance, "currency") and account.balance.currency:
                                currency = account.balance.currency
                            elif hasattr(account, "currency") and account.currency:
                                currency = account.currency

                            # Additional currency validation for known foreign currencies
                            if currency in [
                                "JPY",
                                "USD",
                                "GBP",
                                ...
                                self.logger.debug(f"Found foreign currency account: {account.description} ({currency})")
                            ...
                            self.logger.debug(f"Adding account to results: {account_data}")
                        ...
                        self.logger.exception(f"Error processing account {i + 1}: {error_details}")
            ...
            self.logger.exception(f"Failed to retrieve accounts: {error_details}")
            raise

    def create_or_restore_api_context(self):
        """Create or restore Bunq API context from configuration file."""
        try:
            if Path(self.configuration_file).is_file():
                ...
                self.logger.debug("Configuration file not found, creating new API context")
                hostname = socket.gethostname()
                self.logger.debug(f"Using hostname: {hostname}")
                api_context = ApiContext.create(ApiEnvironmentType.PRODUCTION, self.api_key, hostname)  # type: ignore[arg-type]
            ...
            error_details = {
                ...
                "file_exists": Path(self.configuration_file).is_file(),
                "stack_trace": traceback.format_exc(),
            }
            self.logger.exception(f"Failed to create or restore API context: {error_details}")
            raise
    ...
    def retrieve_accounts_safe(self):
        ...
        try:
            ...
            self.logger.exception("Account retrieval failed")
    ...
    def _patch_bunq_float_adapter(self):
        ...
        try:
            ...
            def safe_float_deserialize(target_class, string):
                if string is None:
            ...
            # Patch the method
            float_adapter.FloatAdapter.deserialize = staticmethod(safe_float_deserialize)
    ...
    def get_alias(account):
        """Get IBAN alias from account object."""
        try:
            ...
            logger.warning(
                f"No IBAN alias found for account: {account.description if hasattr(account, 'description') else 'Unknown'}"
            )
            return None
            ...
            logger.exception(f"Error getting alias for account: {error_details}")
            return None

    @staticmethod
    def get_account_from_type(monetary_account: MonetaryAccountApiObject):
        """Extract account object from MonetaryAccountApiObject."""
        try:
            ...
            elif monetary_account.MonetaryAccountExternal is not None:
                return monetary_account.MonetaryAccountExternal
            elif monetary_account.MonetaryAccountInvestment is not None:
                return monetary_account.MonetaryAccountInvestment

            # Return None for unknown account types without logging
            return None
            ...
            logger.exception(f"Error determining account type: {error_details}")
            return None
